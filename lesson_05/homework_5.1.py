...
search_year, search_engine_volume, search_price = search_criteria
result_dict = {}
for key, value in car_data.items():
    if value[1] >= search_year and value[2] >= search_engine_volume and value[4] <= search_price:
        result_dict[key] = value


sorted_result_dict = dict(
    sorted(result_dict.items(), key=lambda item: item[1][-1])
)

for key, value in list(sorted_result_dict.items())[:5]:
    print(key, value)
