...
def get_country(first_name=None):
    countries = [
        "India", "USA", "Canada", "UK", "Australia", "Germany",
        "France", "Japan", "Brazil", "Italy", "Spain", "China",
        "Ukraine", "Mexico", "South Africa"
    ]
...
def get_address():
	...
	with open(full_path('data.csv'), 'r', newline='', encoding='utf-8') as f:
		addrFile = list(csv.reader(f))[1:]

	for i, field in enumerate(addrParam, start=4):
		allAddrs = []
		...
		if allAddrs:
			...
			full_addr.append(f"Unknown {field}")
